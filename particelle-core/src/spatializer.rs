...
impl Spatializer for AmplitudePanner {
    fn distribute(
        ...
        let mut sum_sq = 0.0;
        for (i, out_gain) in out_gains.iter_mut().enumerate().take(n) {
...
impl HrtfSpatializer {
    pub fn new(layout: AudioLayout) -> Self {
        ...
        assert_eq!(
            layout.channels.len(),
            2,
            "HRTF Spatializer requires exactly 2 output channels."
        );
...
impl AmbisonicEncoder {
    pub fn new(layout: AudioLayout) -> Self {
        ...
        let order = match channels {
            ...
            16 => 3,
            _ => panic!(
                "Ambisonic layout must have 1, 4, 9, or 16 channels for 0th-3rd order. Got {}.",
                channels
            ),
        };
...
impl Spatializer for AmbisonicEncoder {
    fn distribute(
        ...
        if out_gains.len() < self.channels {
            return;
        }
        ...
        if self.order >= 3 {
            ...
            out_gains[15] = root5_8 * x * (x * x - 3.0 * y * y) * radiation_gain;
            // P
        }
    }
...
mod tests {
    ...
    fn test_amplitude_panner_stereo() {
        ...
        panner.distribute(
            Vec3::from_az_el(-30.0, 0.0),
            Vec3::from_az_el(0.0, 0.0),
            1.0,
            0.1,
            &mut gains,
        );
    ...
    fn test_ambisonic_encoder_1st_order() {
        ...
        for i in 0..4 {
            chs.push(crate::layout::ChannelMeta {
                name: format!("ACN{}", i),
                position: crate::layout::SpeakerPosition::Cartesian {
                    x: 0.0,
                    y: 0.0,
                    z: 0.0,
                },
