...
fn normalize_window(window: &mut serde_yaml::Value, report: &mut MigrationReport) {
    ...
    let has_tukey_alpha = mapping.contains_key(value_key("alpha"));
    let has_planck_epsilon = mapping.contains_key(value_key("epsilon"));
    let has_dpss_bw = mapping.contains_key(value_key("half_bandwidth"));
    ...
    match kind.as_deref() {
        ...
        Some("planck_taper") if !has_planck_epsilon => {
            mapping.insert(
                value_key("epsilon"),
