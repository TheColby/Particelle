...
fn normalize_window(window: &mut serde_yaml::Value, report: &mut MigrationReport) {
    ...
    let needs_tukey_alpha = !mapping.contains_key(value_key("alpha"));
    let needs_planck_epsilon = !mapping.contains_key(value_key("epsilon"));
    let needs_dpss_half_bandwidth = !mapping.contains_key(value_key("half_bandwidth"));

    match kind.as_str() {
        "tukey" if needs_tukey_alpha => {
            mapping.insert(
                value_key("alpha"),
        ...
        "planck_taper" if needs_planck_epsilon => {
            mapping.insert(
                value_key("epsilon"),
        ...
        "dpss" if needs_dpss_half_bandwidth => {
            mapping.insert(
