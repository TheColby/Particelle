...
fn cmd_render(
    ...
    while frames_rendered < total_frames {
        ...
        if is_tty && last_update.elapsed() >= update_interval {
            ...
            let width: usize = 30;
            let filled_clamped = ((percent / 100.0) * (width as f64)) as usize;
            let filled_clamped = filled_clamped.min(width);
            let empty = width.saturating_sub(filled_clamped);
            let bar = format!("{}{}", "█".repeat(filled_clamped), "░".repeat(empty));
