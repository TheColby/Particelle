...
fn cmd_render(
    ...
    while frames_rendered < total_frames {
        ...
        if last_update.elapsed().as_millis() > 100 && std::io::stderr().is_terminal() {
            let percentage = (frames_rendered as f64 / total_frames as f64 * 100.0) as u32;
            eprint!("\r\x1b[2K [ {:>3}% ] Rendering...", percentage);
