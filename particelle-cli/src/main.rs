    ...
    for c in &config.clouds {
        ...
        loop {
            ...
            for ch in 0..reader.n_channels {
                full_source[ch][frame_idx..frame_idx + read]
                    .copy_from_slice(&block.channels[ch][..read]);
            }
            frame_idx += read;
        }
...
fn cmd_validate(patch_path: &str) -> Result<()> {
    ...
    let config: particelle_schema::ParticelleConfig =
        serde_yaml::from_str(&yaml).with_context(|| "YAML parse error")?;
...
fn cmd_run(patch: &str) -> Result<()> {
    let yaml =
        std::fs::read_to_string(patch).with_context(|| format!("Cannot read '{}'", patch))?;
    let config: particelle_schema::ParticelleConfig =
        serde_yaml::from_str(&yaml).with_context(|| "YAML parse error")?;
    let errors = particelle_schema::validate(&config);
    if !errors.is_empty() {
                ...
                Ok(event) => {
                    ...
                    if matches!(event.kind, EventKind::Modify(_)) {
                        if event.paths.iter().any(|p| p == &patch_path) {
                            ...
                            let new_config: particelle_schema::ParticelleConfig =
