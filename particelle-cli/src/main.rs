...
fn cmd_render(
    ...
    while frames_rendered < total_frames {
        ...
        if is_terminal && last_progress_time.elapsed() >= Duration::from_millis(100) {
            let progress = (frames_rendered as f64 / total_frames as f64) * 100.0;
            eprint!("\r\x1b[2KRendering... {:.1}%", progress);
