...
fn cmd_render(
    ...
    let ui_throttle = Duration::from_millis(100);
    ...
    while frames_rendered < total_frames {
        ...
        if is_tty && last_ui_update.elapsed() >= ui_throttle {
            let pct = (frames_rendered as f64 / total_frames as f64) * 100.0;
            eprint!("\r\x1b[2K[ Rendering... {:>5.1}% ]", pct);
