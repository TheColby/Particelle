    ...
    for m in 0..n_mels {
            ...
            .skip(f_left)
        {
            if f_center > f_left && k < n_fft / 2 {
                *weight = (k - f_left) as f64 / (f_center - f_left) as f64;
            }
        }
        for (k, weight) in filterbank[m]
            .iter_mut()
            ...
            .skip(f_center)
        {
            if f_right > f_center && k < n_fft / 2 {
                *weight = (f_right - k) as f64 / (f_right - f_center) as f64;
            }
        }
    }
    filterbank
...
pub fn extract_mfcc(config: &SpectralConfig, audio: &[f64], coeff: usize) -> Vec<f64> {
    if audio.is_empty() || coeff >= N_MFCC {
        return Vec::new();
    }
    ...
    while start + config.window_size <= audio.len() {
        windowed_power_spectrum(
            &audio[start..start + config.window_size],
            &fft,
            &mut complex_buf,
        );
        ...
        let mut c = 0.0;
        for (m, energy) in mel_energies.iter().enumerate().take(N_MELS) {
            c += *energy
...
pub fn extract_mfcc1(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 1)
}
pub fn extract_mfcc2(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 2)
}
pub fn extract_mfcc3(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 3)
}
pub fn extract_mfcc4(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 4)
}
pub fn extract_mfcc5(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 5)
}
pub fn extract_mfcc6(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 6)
}
pub fn extract_mfcc7(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 7)
}
pub fn extract_mfcc8(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 8)
}
pub fn extract_mfcc9(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 9)
}
pub fn extract_mfcc10(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 10)
}
pub fn extract_mfcc11(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 11)
}
pub fn extract_mfcc12(config: &SpectralConfig, audio: &[f64]) -> Vec<f64> {
    extract_mfcc(config, audio, 12)
}
